...
lifes = 3
words = ['кот', 'секрет', 'пиво', 'сосиска', 'пароль', ',банан', 'слово', 'взлом', 'жопа', 'книга', 'нигер', 'хрен',
         'бесперспектяковый', 'облом', 'атака']
print('Привет, я Денис Никольский и ты попал в мою игру. '
      ...
      ' Но неожиданно ты теряешь сознание и оказываешься в непонятной комнате. '
      'У тебя есть три пути: вправо,влево,вперед')
...
print(' ')
print(
    'Ты можешь сыграть в небольшую рулетку. Каждый раз, когда ты пишешь "+", ты получаешь случайную прибавку '
    'к здоровью от 1 до 5 к начальному здоровье (начальное здоровье равно 10). '
    'Если твоё здоровье будет больше 20, то ты начнешь игру с 5 очками здоровья.'
...
print('Тебе нужно найти ', quest,
      'загадок. Когда ты решишь их ты выберишься из неизвестного места и выиграешь. '
      'Местоположение загадок неизввестно.')
...
while quest_completed != length:
    ...
    if base_room in monster_rooms:
        print(
            'Ты попал к монстру, у тебя два пути: либо же ты бьешься ("атака") с ним, '
            'или же убегаешь ("побег"), теряя одну жизнь')
    ...
    if base_room in quest_rooms:
        flag2 = True
        print(
            'Заходя в комнату, ты увидел сундук. Ты попытался его открыть, но он оказался закрыт. '
            'Но не ожиданно на сундуке образовалась галограмма:'
            '"Я загадал не которое слово из некоторого количества букв (от 3 до 16 букв). Тебе дано 7 попыток, '
            'чтобы отгадать буквы, входящие в это слово. Если ты угадаешь слово, то сундук откроется. '
            'Иначе тебе придётся решать загадку заново..."')
        while flag2:
...
print(
    'Игра окончена! Ты смог выбраться! Когда ты решил последнюю загадку, то всё потменело, '
    'и то снова оказался на марсе. Рядом с тобой оказалась записка:'

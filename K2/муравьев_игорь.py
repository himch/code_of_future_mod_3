...
print('Привет, детектив! Ты попал в пещеру, где находится куча монстров и загадок. Тебе надо найти все улики и не '
      'умереть от лап монстров, чтоб победить в этом страшном квесте...')
print('Ты расследовал дело о бандитах в одном мало населенном городке, но когда ты почти добрался до истины, '
      'бандиты поняли, что не могут дать тебе расскрыть их тайну. Поэтому им пришлось связать тебя и кинуть в пещеру '
      'с монстрами.')
print('У пещеры 2 этажа. Это прямоугольник 2 вдлину на 3 в ширину, всего 6 комнат. Из любой комнаты можно перейти в '
      'соседнюю.')
...
print('У тебя всего три жизни. У тебя 100 здоровья изначально, а твоя сила удара равна 10, чтоб победить тебе надо '
      'выжить и найти 2 улики')
print("Только найдя улики ты сможешь найти выход из пещеры.")
print('Ну пора выбираться, детектив! Я помогу тебе выбраться из пещеры. Нe забудь в каких комнатах прячутся '
      'монстры!!! Даже если ты убил их, это не значит, что они не могут возродиться!!!')
...
while not (life == 0 or ok == 2):
    ...
    if number == 0:
        ...
        monster = 1
        action = input('Что будем делать, детектив? На выбор 2 варианта действий: побег, атака. В противном случае '
                       'монстр(ы) не даст тебе предпринять что-то другое. Если монстр не один, то ты не сможешь '
                       'сбежать!!! Придеться сражаться...').lower()
        print()
        if action == "побег":
            if monster > 1:
                ...
                print('Монстр был всего один, а значит тебе удалось сбежать! В какую комнату сбежим? Ты в комнате 0. '
        ...
        if action == "побег":
            if monster > 1:
                ...
                print('Монстр был всего один, а значит тебе удалось сбежать! В какую комнату сбежим? Либо влево, '
                      'либо вверх')
                continue
        elif action == "атака":
            mon_hp = 27
